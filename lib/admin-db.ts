...
export async function listProfiles():Promise<Profile[]> {
  const result=await query<{id:string;name:string;description:string;access_level:Profile["nivel"];scope:Profile["ambito"];permissions:string[];active:boolean;users_count:number}>(`
  ...
  `);
  return result.rows.map((row)=>({id:row.id,nome:row.name,descricao:row.description,nivel:row.access_level,utilizadoresCount:row.users_count,permissoes:row.permissions,ambito:row.scope,estado:row.active?"activo":"inactivo"}));
}
