...
interface ExpedientRow {
  ...
  origin_unit_id: string; origin_unit_name: string; recipient_unit_name: string;
...
function accessClause(session: AuthSession, startIndex = 1) {
  if (session.perfilNavegacao === "administracao" || session.perfilNavegacao === "secretaria") {
    return { sql: "TRUE", params: [] as unknown[] };
  }
  if (session.perfilNavegacao === "superior") {
    return { sql: `(e.origin_unit_id=$${startIndex} OR e.recipient_unit_id=$${startIndex} OR e.responsible_user_id=$${startIndex + 1})`, params: [session.user.unidadeId, session.user.id] };
  }
  return { sql: `(e.created_by=$${startIndex} OR e.responsible_user_id=$${startIndex})`, params: [session.user.id] };
}

function baseSelect() {
  return `SELECT e.*,ou.name origin_unit_name,ru.name recipient_unit_name,
...
const VIEW_FILTERS: Record<ExpedientView, string> = {
  ...
  inbox: "e.responsible_user_id=__USER__ AND e.status NOT IN ('arquivado','cancelado','recebimento_confirmado')",
...
export async function listExpedients(session: AuthSession, view: ExpedientView = "all") {
  const access = accessClause(session, 1);
...
export async function getExpedient(session: AuthSession, id: string) {
  const access = accessClause(session, 2);
