...
export async function POST(request:NextRequest,{params}:{params:{id:string}}){
  ...
  const access=await query<{status:string;created_by:string;responsible_user_id:string|null;origin_unit_id:string;recipient_unit_id:string}>("SELECT status,created_by,responsible_user_id,origin_unit_id,recipient_unit_id FROM expedients WHERE id=$1",[params.id]);
  const exp=access.rows[0];if(!exp)return NextResponse.json({error:"Expediente nao encontrado."},{status:404});
  const allowed=session.perfilNavegacao==="administracao"||(session.perfilNavegacao==="secretaria"&&exp.status!=="rascunho")||exp.created_by===session.user.id||exp.responsible_user_id===session.user.id||(session.perfilNavegacao==="superior"&&(exp.origin_unit_id===session.user.unidadeId||exp.recipient_unit_id===session.user.unidadeId));
