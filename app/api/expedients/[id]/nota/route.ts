  ...
  try {
    ...
    const result = await transaction(async (client) => {
      ...
      if (input.modo === "sistema") {
        ...
        const stampEntry = resolved.stamp ? JSON.stringify(stampMetadataJson(resolved.stamp, session.user.nome)) : null;
        await client.query(
          `INSERT INTO documents(id,expedient_id,name,document_kind,source,mime_type,size_bytes,page_count,content_html,confidentiality,created_by,stamp_id,stamped,signed,stamp_metadata,signature_metadata,stamps_metadata,signatures_metadata,template_metadata,document_number)
           VALUES($1,$2,$3,'nota','sistema','text/html',$4,1,$5,'interno',$6,$7,$8,true,$9::jsonb,$10::jsonb,$11::jsonb,$12::jsonb,$13::jsonb,$14)`,
          [documentId, exp.id, `Nota - ${exp.protocol}.html`, Buffer.byteLength(clean, "utf8"), clean, session.user.id, resolved.stamp?.id ?? null,
            Boolean(resolved.stamp), stampEntry, signatureEntry, stampEntry ? `[${stampEntry}]` : null, `[${signatureEntry}]`, template ? JSON.stringify(template) : null, documentNumber],
        );
      } else if (file) {
      ...
      await client.query(
        `INSERT INTO timeline_events(expedient_id,event_type,title,description,user_id,unit_id) VALUES($1,'nota','Nota de encaminhamento criada',$2,$3,$4)`,
        [exp.id, input.note?.trim() || `Nota criada sobre ${exp.protocol}.`, session.user.id, session.user.unidadeId],
      );
      ...
      for (const userId of notifyIds) {
        if (userId !== session.user.id) {
          await client.query(
            "INSERT INTO notifications(user_id,notification_type,title,description,expedient_id,urgent) VALUES($1,'tarefa','Nota de encaminhamento criada',$2,$3,false)",
