...
export async function PATCH(request:NextRequest,{params}:{params:{id:string}}){
  ...
  try{const input=await request.json();await transaction(async(client)=>{
    if(input.unidadeId){const unit=await client.query("SELECT 1 FROM organizational_units WHERE id=$1 AND active=true",[input.unidadeId]);if(!unit.rows[0])throw new Error("Seleccione uma unidade activa.");}
    if(input.perfilId){const profile=await client.query("SELECT 1 FROM profiles WHERE id=$1 AND active=true",[input.perfilId]);if(!profile.rows[0])throw new Error("Seleccione um perfil activo.");}
