...
interface DocumentAccessRow {
  ...
  stamp_metadata:PdfStampMetadata|null; signature_metadata:PdfSignatureMetadata|null;
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await query<DocumentAccessRow>(`SELECT d.name,d.mime_type,d.storage_path,d.content_html,d.stamp_metadata,d.signature_metadata,d.template_metadata,e.protocol,e.subject,e.status,e.created_by,e.origin_unit_id,e.recipient_unit_id,e.responsible_user_id FROM documents d JOIN expedients e ON e.id=d.expedient_id WHERE d.id=$1`, [params.id]);
