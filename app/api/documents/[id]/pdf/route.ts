...
interface DocumentAccessRow {
  name:string; mime_type:string|null; storage_path:string|null; content_html:string|null;
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await query<DocumentAccessRow>(`SELECT d.name,d.mime_type,d.storage_path,d.content_html,d.stamps_metadata,d.signatures_metadata,d.template_metadata,e.protocol,e.subject,e.status,e.created_by,e.origin_unit_id,e.recipient_unit_id,e.responsible_user_id FROM documents d JOIN expedients e ON e.id=d.expedient_id WHERE d.id=$1`, [params.id]);
  ...
  try {
    ...
    const pdf = await createDocumentPdf({ name: doc.name, mimeType: doc.mime_type, contentHtml: doc.content_html, sourceFile, protocol: doc.protocol, subject: doc.subject, stamps: doc.stamps_metadata ?? [], signatures: doc.signatures_metadata ?? [], template: doc.template_metadata, institutionName });
